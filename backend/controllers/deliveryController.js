...
import DeliveryAccessCode from '../models/DeliveryAccessCode.js';
...
// @desc    Generate access code (Admin only)
// @route   POST /api/delivery/access-code/generate
// @access  Private/Admin
export const generateDeliveryAccessCode = async (req, res, next) => {
    try {
        ...
        await DeliveryAccessCode.deleteMany({}); // Only keep one active code
        
        const newCode = await DeliveryAccessCode.create({
            code,
            expiresAt
        });
...
// @desc    Get current access code (Admin only)
// @route   GET /api/delivery/access-code
// @access  Private/Admin
export const getDeliveryAccessCode = async (req, res, next) => {
    try {
        const currentCode = await DeliveryAccessCode.findOne();
        if (currentCode) {
...
// @desc    Verify delivery access code (Delivery)
// @route   POST /api/delivery/access-code/verify
// @access  Private/Delivery
export const verifyDeliveryAccessCode = async (req, res, next) => {
    try {
        const { code } = req.body;
...
export const verifyDeliveryAccessCode = async (req, res, next) => {
    try {
        ...
        if (!currentCode) {
            res.status(400);
            return next(new Error('No access code configured by admin'));
        }

        if (currentCode.code !== code.trim()) {
            res.status(400);
            return next(new Error('Invalid access code. Please check the code and try again.'));
        }

        if (Date.now() > new Date(currentCode.expiresAt).getTime()) {
            res.status(403);
            return next(new Error('Access code has expired. Contact Admin.'));
        }
